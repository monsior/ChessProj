...
Game::Game()
{
	window = new sf::RenderWindow(sf::VideoMode(SCREEN_SIZE, SCREEN_SIZE), "Chess");	
...
void Game::CreateBoard()
{
	...
	whiteRect.loadFromFile("Assets/whiteRect.png");
	blackRect.loadFromFile("Assets/blackRect.png");
	...
	/*			rectangle.setFillColor(lightSqure);*/
			...
			{
				rectangle.setTexture(&blackRect);
...
void Game::LoadSprites()
{
	...
	for (int i = 0; i < DIMENSIONS; i++)
	{
		for (int j = 0; j < DIMENSIONS; j++)
		{
			...
			{
			case 'p':
				pieces[i][j] = new Pawn(piecesTex[p], Indexes2Position(sf::Vector2i(i, j)), isWhite, sf::Vector2i(i, j), p);
				break;
			case 'K':
...
void Game::cleanSqure(sf::Vector2i indexes)
{
	...
	{
		squres[indexes.x * DIMENSIONS + indexes.y].setFillColor(sf::Color::White);
	}
	else
	{
		squres[indexes.x * DIMENSIONS + indexes.y].setFillColor(sf::Color::White);
...
void Game::CleanBoard()
{
	for (int i = 0; i < DIMENSIONS; i++)
	{
		for (int j = 0; j < DIMENSIONS; j++)
		{
			...
			{
			case 'p':
				pieces[i][j] = new Pawn(piecesTex[p], Indexes2Position(sf::Vector2i(i, j)), p[0] == 'w', sf::Vector2i(i, j), p);
				break;
			case 'K':
...
void Game::UndoMove(sf::Vector2i indexesBefore, sf::Vector2i indexesAfter)
{
	...
	case(moveTypes::PROMOTION):
	{
		...
		pieces[indexesBefore.x][indexesBefore.y] = new Pawn(piecesTex[type], // create a new pown
